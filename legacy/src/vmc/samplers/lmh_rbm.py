...
class LMHRBM(BaseRBMVMC):
    ...
    def step(self, state, seed, D=0.5, dt=1.0):
        """One step of the ...
        ...
        F = self._driftF_fn(state.positions)
        ...
        logp_prop = self._logp_fn(proposals)

        # Green's function conditioned on proposals
        F_prop = self._driftF_fn(proposals)
        ...
        new_logp = self._logp_fn(new_positions)
